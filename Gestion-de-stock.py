...
window = Tk()
window.title("Gestioner de stock")
...
style.configure("Treeview", background="white", foreground="black", rowheight=20, fieldbackground="grey")

# choisir la couleur de sélection
style.map("Treeview", background=[("selected", "#335F07")], foreground=[("selected", "#94812B")])
...
my_tree["columns"] = ("ID", "Nom", "Description", "Prix", "Quantité", "Num catégorie")
...
my_tree.column("Num catégorie", anchor=CENTER, width=65)
...
my_tree.heading("Num catégorie", text="Num catégorie", anchor=CENTER)

# créer des lignes rayées
my_tree.tag_configure("oddrow", background="#ED7F10")
my_tree.tag_configure("evenrow", background="#1280EF")
...
cat_label = Label(data_frame, text="Num catégorie")
